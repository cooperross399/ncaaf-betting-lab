...
**The pre-registered positive control PASSES — and it passes with no margin in either of the two ways a result can have margin.** On the spread the close beats the open by D = mean(e_open²) − mean(e_close²) = **+3.7147 pts²**, n = **3,851 games** over 76 `(season, week)` clusters, corrected interval **[+0.3819, +7.0475]**, which excludes zero on the pre-registered positive side (z = 3.8547 against 3.4584). In points that is RMSE 15.3841 -> 15.2629, an improvement of **0.1212 points**, 0.79% of RMSE, on n = 3,851.
...
**No power to spare.** The measured D sits *below* the design's own 80% floor of +4.1438 pts² (n = 3,851) — restated in points, the smallest forecast improvement this design could reliably detect is 0.1347 RMSE points and it measured 0.1212. That is arithmetically consistent with the interval excluding zero, but it means a replication would clear the corrected bar well under four times in five.

**Robustness, and this is what the re-run changed.** The pass leans on a thin tail of games, and part of that tail was never a market move at all but a defect in the feed. See *The opener is dirtier than the study assumed* below: dropping **32 of 3,851 games (0.83%)** whose raw rows cannot be read as a home handicap takes D to +3.3139 pts² (n = 3,821) with an interval of [+0.0811, +6.5468] — which **still excludes zero**. On the pre-fix table the same exclusion collapsed the interval across zero, and that fragility is why the study was re-run; it is gone.
...
**A pass is not an edge and is not quoted as one.** 0.1212 RMSE points is roughly a tenth of the 1.5 points a -110 price needs to clear the vig. Nothing here is bettable, no bet was placed and none was automated.

| Control | n (games) | Clusters | D (pts²) | SE | Corrected interval | Excludes zero | 80% floor |
|:---|---:|---:|---:|---:|:---|:---|---:|
| spread — **the registered control** | 3,851 | 76 | +3.7147 | 0.9637 | [+0.3819, +7.0475] | yes, positive side | +4.1438 |
| total — *not registered* | 3,862 | 77 | +4.8750 | 1.1902 | [+0.7588, +8.9912] | yes, positive side | +5.1179 |
| spread, moved games only — *not registered* | 3,296 | 76 | +4.2465 | 1.1368 | [+0.3149, +8.1780] | yes, positive side | +4.8884 |

The total market is reported because it was run, not because it was registered: RMSE 15.9085 -> 15.7545 on n = 3,862. Its measured D also sits below its own floor of +5.1179. The moved-games row exists only to show the pooled D is not an artefact of the 555 games (14.4% of n = 3,851) whose line never moved and whose paired D is exactly zero.
...
**No demonstrated edge.** Slope **-0.014738**, cluster-robust SE 0.007667, n = **3,115 games** over 62 clusters, corrected interval **[-0.0413, +0.0118]**, which includes zero. The sign is NEGATIVE against a registered POSITIVE, but zero is inside the interval, so no anti-correlation is demonstrated either.

**Interpretable, not an absence — and it goes further than a null.** The detectable floor is 0.032968 (n = 3,115), which sits below the 0.0502 a half-point tick of movement at the top decile of disagreement would represent, so the design could have seen an effect of the size that would matter. Separately from the null, the interval's upper bound of +0.011778 corresponds to at most **+0.12 points of line move** at the top decile (9.97 points, sd(disagree_open) = 7.7878, n = 3,115). A market revision toward these ratings of even a quarter-point tick (0.0251) is ruled out. "No demonstrated edge" and "ruled out" are different claims and both are available here, so both are made.
...
The Step 5 instrument with the opener in place of the close. `profitable_slope = 1.5 / (1.28 x sd(disagree_open) 7.7731) = **0.1508**` — the formula was registered in advance and the value is measured on this sample. Disagreement with the opener is *tighter* than with the close (7.7731 against the published 8.19), which raises the bar rather than lowering it.

| Split | n | Clusters | Slope | SE | Corrected interval | Reading | 80% floor | Own threshold |
|:---|---:|---:|---:|---:|:---|:---|---:|---:|
| all games | 3,115 | 62 | -0.0455 | 0.0396 | [-0.1824, +0.0915] | **no demonstrated edge** | 0.1703 | 0.1508 |
| early season (weeks 1-4) | 939 | 16 | -0.1486 | 0.0565 | [-0.3439, +0.0466] | **no demonstrated edge** | 0.2428 | 0.1438 |
| late season (weeks 5+) | 2,176 | 46 | +0.0055 | 0.0482 | [-0.1611, +0.1721] | **no demonstrated edge** | 0.2071 | 0.1545 |
...
1. **A paying slope is ruled out on all games** (upper bound +0.0915 < 0.1508) and on the early-season split (+0.0466 < 0.1438). It is **not** ruled out late season: upper bound +0.1721 against its own 0.1545, n = 2,176. Nothing should be claimed there in either direction — the same split Step 5 left unresolved against the close.
2. **The 80% power criterion fails everywhere.** Every floor (0.1703 / 0.2428 / 0.2071) sits above the threshold that would matter. So the all-games ruling-out is the realized interval speaking, not a design that could be trusted in advance to have seen a paying slope. **Report the ruling-out and the power failure together; either alone misleads.**
3. **The direction failed.** Registered POSITIVE, measured -0.0455 on all games and -0.1486 early season (n = 939) — the split the pre-registration named as the likeliest place for a soft opener. The intervals include zero, so this is a failed prediction and not a demonstrated reversal, and it must not be re-narrated as a finding about the market overreacting to ratings.
...
**H2, all games — to make the null a finding rather than a realized-interval statement.** The floor must fall from 0.1703 to the 0.1508 that would pay, so the SE must fall from 0.0396 to 0.0351. If the cluster-robust SE scales as 1/sqrt(n), that is about **3,975 games**, roughly **1.1 more seasons** of FBS-vs-FBS football beyond the 3,115 in hand. The ratchet bites here: testing again adds hypotheses, which raises the critical value, which raises the floor — so the requirement grows slightly every time it is re-asked, and the honest figure is 'at least' that many.

**H2, late season — the only split that has ruled nothing out.** Upper bound +0.1721 on n = 2,176, above its own 0.1545. It needs the same order of additional seasons, and it is the one place in this study where a future look is defensible rather than a second bite.
...
**H1 — bounded, and the remaining gap is small.** The design already rules out a quarter-point revision toward the ratings. To resolve a quarter-point *effect* rather than bound it, the floor would need to reach 0.0251, about **5,383 games** (2.9 more seasons). Worth noting only because it is a CLV question; there is still no price on the move.
